...
export default function Contacts() {
  ...
  return (
    <div className="text-white mt-5">
      <Card
        isBlurred
        className="w-2/3 m-auto mt-4 p-4 px-6 text-white"
      ...
      >
        ...
        >
          <form className="w-full">
            <MyInput
                ...
                value={nameValue}
                item={{name:'text',type:'text',placeholder:'John',label:'Name'}}/>
            <MyInput
              className='mt-3 w-72'
              handleChange={(e)=>setEmailValue(e.target.value)}
              value={emailValue}
              item={{name:'email',type:'email',placeholder:'you@example.com',label:'Email'}}/>
            ...
            <div className="relative mt-3">
              <textarea
                ...
                className={`peer/email w-72 h-24 resize-none self-center text-white placeholder-slate-300 mt-6 rounded-lg border-2 text-xs p-2 bg-inherit focus:outline-none focus:border-cyan-300 scrollbar-hide ${
                  messValue ? "border-cyan-300" : "border-white border-opacity-35"
                }`}
                type="text"
            ...
            <div className="flex mt-3">
              <input onChange={()=>{setAgreeValue(!agreeValue)}} checked={agreeValue} value={agreeValue} type="checkbox" className="peer bg-white text-white"></input>
        ...
        <CardFooter className="justify-center">
          <Button isDisabled={!(agreeValue && nameValue && emailValue && messValue)} className="bg-orange rounded-full text-white h-8 w-36 hover:bg-white hover:text-orange">
            SEND
          </Button>
        </CardFooter>
      </Card>
