  ...
  return (
    <div className="h-screen">
      {/* {coins.filter(coin => coin.currencyName === router.query.coin).map((coin) => ( */}
        ...
        >
          <h1 className="font-bold py-2">Coin: &ensp;{router.query.currencyName}</h1>
          <h1 className="font-bold py-2">Network: &ensp;{router.query.network}</h1>
          <h1 className="font-bold py-2">Address: &emsp;<u>{router.query.address}</u></h1>
        </div>
      {/* ))} */}
    </div>
  );
