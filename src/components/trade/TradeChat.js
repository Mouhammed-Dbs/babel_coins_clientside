export default function TradeChat() {
  return (
    <div className="px-2 h-full">
      <ul className="bg-white/85 dark:bg-default-200/50 rounded-sm w-full h-[290px] overflow-scroll no-scrollbar p-1">
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg bjlbl ljbljb bhgpug bgipgbp pigoug</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg bjlbl ljbljb bhgpug</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg bjlbl ljbljb bhgpug bgipgbp pigoug</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg bjlbl ljbljb bhgpug</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg bjlbl ljbljb bhgpug bgipgbp pigoug</span>
        </li>
        <li className="flex text-[11px] px-1 gap-1">
          <span className="font-bold">b7808680:</span>
          <span>hi, my name is mouhyg bjlbl ljbljb bhgpug</span>
        </li>
      </ul>
    </div>
  );
}
